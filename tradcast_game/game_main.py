...
def notify_score_update_thread(
    ...
    if DEBUG:
        print(f"[DEBUG] POST score update -> {MAIN_API_URL}/internal/update_score  fid={fid} profit={profit:.2f}")
    payload = {
        "fid": fid,
        "profit": profit,
        "secret": SECRET_KEY,
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 200:
                if DEBUG:
                    with _stats_lock:
                        _stats["score_notifications_sent"] += 1
                        print(f"[DEBUG] POST score update <- 200 OK  fid={fid}")
                return
    ...
    if DEBUG:
        with _stats_lock:
            _stats["score_notifications_failed"] += 1
...
async def debug_info():
    ...
    return {
        "server": "tradcast_game",
        "debug": True,
        "uptime_seconds": round(uptime, 1),
        "uptime_human": f"{int(uptime // 3600)}h {int((uptime % 3600) // 60)}m {int(uptime % 60)}s",
        "websocket": {
            "connections_total": snapshot["ws_connections_total"],
            "connections_active": snapshot["ws_connections_active"],
            "auth_successes": snapshot["ws_auth_successes"],
            "auth_failures": snapshot["ws_auth_failures"],
            "no_energy_rejections": snapshot["ws_no_energy"],
        },
        "games": {
            "sessions_saved": snapshot["games_saved"],
            "parquet_datasets_loaded": len(spike_df_map),
        },
        "score_sync": {
            "notifications_sent_ok": snapshot["score_notifications_sent"],
            "notifications_failed": snapshot["score_notifications_failed"],
        },
        "http": {
            "total_requests": snapshot["http_requests_total"],
            "tracker_calls_to_main": snapshot["tracker_calls"],
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        await _handle_ws_session(websocket)
    finally:
        if DEBUG:
            with _stats_lock:
                _stats["ws_connections_active"] -= 1
...
async def _handle_ws_session(websocket: WebSocket):
    trade_actions = []
    ...
    try:
        ...
        try:
            ...
            try:
                daily_games = await firestore_manager.handle_daily_games(fid)
                doc_id = await firestore_manager.track_user(fid=fid, wallet=fid, location=SERVER_LOC, daily_games=daily_games)

            except Exception as e:
                print('error handle daily games then track user: ', e)
                return
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("game_main:game_app", port=6002, reload=False)
